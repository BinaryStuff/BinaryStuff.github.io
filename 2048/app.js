...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addColours() {
        for (let i = 0; i < squares.length; i++) {
            ...
            else if (squares[i].innerHTML == 1024) squares[i].style.backgroundColor = '#f7b500'
            else if (squares[i].innerHTML == 2048) squares[i].style.backgroundColor = '#ff7d7d'
        }
    }
    addColours()
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'l' || e.key === 'L') {
            document.getElementById('ai-buttons').style.display = aiActive ? 'none' : 'block';
        }
    });
});
</script>
